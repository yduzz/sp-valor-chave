...
export default async function handler(
  ...
  try {
     ...
     * number=100
     *
     * Quando o número é informado, o PostgreSQL
     * também restringe a consulta ao logradouro
     * + número antes do limite de registros.
     */
    if (path === "/api/property-search" && req.method === "GET") {
       ...
       * Filtro do número.
       *
       * O espaço antes do número reduz falsos positivos,
       * como procurar 100 e encontrar 1000.
       *
       * Também testamos a versão sem zeros à esquerda.
       *
       * Exemplos:
       * 100  -> " 100"
       * 0100 -> " 0100" ou " 100"
       */
      if (numberParam) {
        const numberIndex = values.length + 1;

        const normalizedNumber =
          numberParam.replace(/^0+/, "") || "0";

        const numberIndex2 = numberIndex + 1;

        conditions.push(
          `(address ILIKE $${numberIndex} OR address ILIKE $${numberIndex2})`
        );
