...
const INPUT = path.resolve("data/itbi/itbi_2007.xlsx");
const OUTPUT = path.resolve("data/itbi/converted/itbi_2007.csv");
...
const MONTH_SHEETS = workbook.SheetNames.filter((name) =>
  /^(JAN|FEV|MAR|ABR|MAI|JUN|JUL|AGO|SET|OUT|NOV|DEZ)/i.test(name)
);

console.log("\nAbas mensais encontradas:");
console.log(MONTH_SHEETS);

if (MONTH_SHEETS.length !== 12) {
  console.error(
    `ATENÇÃO: eram esperadas 12 abas mensais, mas foram encontradas ${MONTH_SHEETS.length}.`
  );
  process.exit(1);
}
